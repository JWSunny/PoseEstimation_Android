...
public class DrawView extends View {
    ...
    /**
     * 输入的为192*192的图,然后按照比例放大
     * 先按ratio放大,再按机器实际尺寸放大
     *
     * @param point 2*14
     */
    public void setDrawPoint(float[][] point, Rect rect, float ratio, float rateX, float rateY) {
        mDrawPoint.clear();

        float tempX, tempY;
        for (int i = 0; i < 14; i++) {
            tempX = (point[0][i]*(rect.right-rect.left) / ratio / 192 + rect.left) * rateX / mRatioX;
            tempY = (point[1][i]*(rect.bottom-rect.top) / ratio / 192 + rect.top) * rateY / mRatioY;
